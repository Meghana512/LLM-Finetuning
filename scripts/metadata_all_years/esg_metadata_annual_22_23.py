# ESG METADATA EXTRACTION PIPELINE FOR ANNUAL REPORTS 22-23
# =========================================================
# Fixes vs v3:
...
OUTPUT_CSV   = "metadata_dataset_24-25.csv"
OUTPUT_JSONL = "metadata_dataset_24-25.jsonl"
...
def _normalise(name: str) -> str:
    ...
    # remove years
    name = re.sub(r'20\d{2}\s*[-_]?\s*\d{2}', ' ', name)

    # remove common report words
    remove_words = [
...
def extract_company_name(raw_text):
    ...
    for line in lines[:40]:
        ...
        if re.search(r'(limited|ltd|bank|industries|technologies|corp)',
                     line, re.I):
...
def process_pdf(file):
    try:
        ...
        pdf_company_name = extract_company_name(raw)
        lookup_name = pdf_company_name if pdf_company_name else company
        if not clean:
            return None
        return {
            "company"        : company,
            "nse_symbol"     : _find_nse_symbol(lookup_name) or "Unknown",
